...
namespace DALTUDTXD_68TH4_PMTKVK_REVITAPI.ViewModels
{
    public class DuLieuViewModel : INotifyPropertyChanged, IDataErrorInfo
    {
        ...
        {
            NavigateAction?.Invoke(parameter?.ToString());
        }
        ...
        public void ProcessSelectedSlab(Reference reference)
        {
            if (reference != null)
            {
                ...
                if (element is Floor floor)
                {
                    ...
                    SlabThickness = thicknessMm;

                    Parameter areaParam = floor.get_Parameter(BuiltInParameter.HOST_AREA_COMPUTED);
                    if (areaParam != null)
                    {
                        double areaSqFt = areaParam.AsDouble();
                        SelectedSlabArea = areaSqFt * 0.092903; // convert sq ft to m2
                    }
                    else
                    {
                        SelectedSlabArea = 100.0; // Fallback
                    }
                    ...
                    if (perimeterParam != null)
                    {
                        ...
                        SelectedSlabPerimeter = perimeterFt * 0.3048; // convert ft to m
                    }
                    else
                    {
                        SelectedSlabPerimeter = 40.0; // Fallback
                    }
                    
                    AutoDesignFormwork();

                    RevitVersionStatus = $"Đã chọn sàn: {floor.Name} | Chiều dày: {thicknessMm:F0} mm";

                    _dialogService.ShowInfo(
                        $"Đã chọn thành công sàn từ Revit!\n" +
                        $"- Tên sàn: {floor.Name}\n" +
                        $"- Chiều dày: {thicknessMm:F0} mm\n\n" +
                        $"Hệ thống đã TỰ ĐỘNG TÍNH TOÁN và đưa ra phương án bố trí ván khuôn an toàn, tối ưu nhất dựa trên chiều dày sàn này.",
